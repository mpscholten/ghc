...
import GHC.Data.FastString (mkFastString, mkFastStringByteString)
        ...
        RealSrcSpan rsp _ -> do
          ...
          (bEnd', inPrag') <- case tok of
            ...
            ITline_prag _ -> tryOrElse (bEnd, inPragDef) $ do
              ...
              let newLoc = mkRealSrcLoc (mkFastStringByteString file) (fromIntegral line - 1) (srcSpanEndCol spF)
              (bEnd'', _) <- lift getInput
