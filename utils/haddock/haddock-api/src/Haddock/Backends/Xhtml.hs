      ...
      where
        html =
          linkedAnchor (groupId (LText.fromStrict id0))
            << docToHtmlNoAnchors (Just (T.unpack id0)) pkg qual (mkMeta doc)
...
processExport summary _ _ pkg qual (ExportGroup lev id0 doc) =
  nothingIf summary $ groupHeading lev (LText.fromStrict id0) << docToHtmlNoAnchors (Just (T.unpack id0)) pkg qual (mkMeta doc)
