...
-- | Run the assembler with stdin piped, for overlapping codegen with
-- assembler startup. The action receives the stdin handle to write to.
withAsPiped :: Logger -> DynFlags -> [Option] -> (Handle -> IO a) -> IO a
withAsPiped logger dflags args action = traceSystoolCommand logger "as" $ do
  let (p,args0) = pgm_a dflags
      args1 = map Option (getOpts dflags opt_a)
      args2 = args0 ++ args1 ++ args
  mb_env <- getGccEnv args2
