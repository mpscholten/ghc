...
import GHC.Driver.CodeOutput (asmPlatformOpts, canPipeAsm)
      ...
      HscRecomp { hscs_guts = cgguts,
        ...
        -> if not (backendGeneratesCode (backend dflags)) then
             ...
             do
              ...
              -- Compute the .o path and decide whether to pipe asm
              ...
              let pipe_asm = canPipeAsm (Just hsc_env) dflags (stop_phase pipe_env)
                  mb_pipe_obj = if pipe_asm then Just obj_fn else Nothing
              ...
              let asm_output = if pipe_asm then AsmPiped obj_fn else AsmToFile outputFilename
